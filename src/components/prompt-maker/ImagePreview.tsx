...
export const ImagePreview = ({
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <label htmlFor="reference-image" className={`cursor-pointer block ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
              <div className="h-14 w-14 rounded-md bg-white/5 backdrop-blur border border-white/10 p-3 hover:bg-white/10 flex items-center justify-center">
                <svg width="20" height="20" viewBox="0 0 30 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-white/70">
  ...
  return (
    <button 
      type="button" 
      className={`h-14 w-14 rounded-md bg-white/5 backdrop-blur border border-white/10 p-1 hover:bg-white/10 group relative ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`} 
      onClick={onRemove} 
