            ...
            <TooltipTrigger asChild>
              {!imagePreview ? (
                <div 
                  ...
                  className={`h-8 w-8 rounded-md bg-[#242038] border border-purple-500/30 flex items-center justify-center ${workflow === 'no-reference' ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:bg-[#302b45]'} transition-colors`}
                >
                ...
                <button 
                  type="button" 
                  className={`h-8 w-8 rounded-md bg-[#242038] border border-purple-500/30 p-1 group relative hover:bg-[#302b45] transition-colors ${isUploading || isProcessing || workflow === 'no-reference' ? 'opacity-50 cursor-not-allowed' : ''}`} 
                  onClick={removeImage} 
