          ...
          <div className="absolute top-0 bottom-0 left-0 w-24 flex flex-col justify-center items-center gap-4 border-r border-white/10 z-10 px-0 py-0 my-0 mx-0">
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  {!imagePreview ? (
                    <div 
                      ...
                      className={`h-12 w-12 rounded-md bg-white/5 backdrop-blur border border-white/10 p-3 hover:bg-white/10 transition-colors flex items-center justify-center ${workflow === 'no-reference' ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                    >
                    ...
                    <button 
                      type="button" 
                      className={`h-12 w-12 rounded-md bg-white/5 backdrop-blur border border-white/10 p-1 hover:bg-white/10 group relative ${isUploading || isProcessing || workflow === 'no-reference' ? 'opacity-50 cursor-not-allowed' : ''}`} 
                      onClick={removeImage} 
                    ...
                    >
                      ...
                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="absolute inset-0 m-auto h-6 w-6 text-white opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <TooltipContent side="bottom">
                  <p>{imagePreview ? "Remove image" : "Add image"}</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <div 
                    className={`h-12 w-12 rounded-md bg-white/5 backdrop-blur border border-white/10 hover:bg-white/10 transition-colors flex items-center justify-center ${isImprovingPrompt || !prompt.trim() ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
